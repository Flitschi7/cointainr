...
<div class="cache-health-indicator">
	<TransitionWrapper
		isLoading={!$cacheHealth || $cacheHealth.status === undefined}
		transitionType="fade"
	>
		...
		{#if $cacheHealth && $cacheHealth.status !== undefined}
			<div class="flex items-center gap-2">
				<div class="flex items-center">
					<div class={`h-3 w-3 rounded-full ${healthStatusClass}`}></div>
					<span class="ml-2 font-medium">Cache Health: {healthStatusText}</span>
				</div>
